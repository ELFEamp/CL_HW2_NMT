...
class Encoder(nn.Module):
    def __init__(self, config):
        ...
        sinusoid_table = torch.FloatTensor(get_sinusoid_encoding_table(self.config['max_src_seq_length']+1, self.config['d_model']))
...
class Decoder(nn.Module):
    def __init__(self, config):
        ...
        sinusoid_table = torch.FloatTensor(get_sinusoid_encoding_table(self.config['max_tgt_seq_length']+1, self.config['d_model']))
        self.pos_emb = nn.Embedding.from_pretrained(sinusoid_table,freeze=True)
